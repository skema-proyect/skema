  ...
  return (
    <div className="flex flex-col h-full bg-s-sidebar text-s-sidebar-text select-none relative">
      ...
      <div className="px-3 py-3 border-b border-s-sidebar-border">
        <p className="sb-section text-[12px] uppercase tracking-widest text-s-sidebar-muted mb-2 px-1">Servicios</p>
        <div className="space-y-0.5">
          {SERVICES.map(({ icon: Icon, label, prompt }) => (
            <button
              key={label}
              onClick={() => { onServiceSelect(prompt); onClose?.(); }}
              className="sb-item w-full flex items-center gap-3 lg:gap-2.5 px-2 py-2.5 rounded text-[15px] text-s-sidebar-muted hover:bg-s-sidebar-hover hover:text-s-sidebar-text transition-colors text-left"
            >
              <Icon size={15} className="sb-icon flex-shrink-0" />
              {label}
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-2 py-3 space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between px-2 mb-1">
            <span className="sb-section text-[12px] uppercase tracking-widest text-s-sidebar-muted font-medium">Proyectos</span>
              ...
              <div key={p.id}>
                <div className="flex items-center gap-1 px-2 py-2 lg:py-1.5 rounded group">
                  ...
                  <button
                    ...
                    className="sb-item flex-1 text-left text-[15px] truncate text-s-sidebar-text hover:text-white transition-colors min-w-0"
                  >
                    {p.name}
                  </button>
        ...
        {loose.length > 0 && (
          <div>
            <p className="sb-section px-2 text-[12px] uppercase tracking-widest text-s-sidebar-muted font-medium mb-1">Recientes</p>
            {loose.map(c => (
              <ConvItem key={c.id} conv={c} active={currentConvId === c.id}
                onSelect={() => { onSelectConv(c.id); navigate("/"); onClose?.(); }}
      ...
      <div className="border-t border-s-sidebar-border px-2 py-3 space-y-0.5">
        <button onClick={() => goTo("/notas")}
          className={`sb-item w-full flex items-center gap-2.5 px-2 py-2 rounded text-[16px] transition-colors ${
          ...
          <StickyNote size={15} className="sb-icon flex-shrink-0" /> Notas
        </button>
        <button onClick={() => goTo("/agenda")}
          ...
          <Calendar size={15} className="sb-icon flex-shrink-0" /> Agenda
        </button>
        ...
        {profile?.role === "admin" && (
          <button onClick={() => goTo("/admin")}
            ...
            <ShieldCheck size={15} className="sb-icon flex-shrink-0" /> Admin
          </button>
        )}

        {!installed && !isStandalone && (
          <button onClick={handleInstall}
            className="sb-item lg:hidden w-full flex items-center gap-2.5 px-2 py-2 rounded text-[16px] text-s-sidebar-muted hover:bg-s-sidebar-hover hover:text-s-sidebar-text transition-colors">
            <Download size={15} className="sb-icon flex-shrink-0" /> Instalar app
          </button>
        )}
      </div>
...
function ConvItem({ conv, active, onSelect, onDelete, onMove, projects, indent = false }: {
  ...
  return (
    <div ref={ref} onClick={onSelect}
      className={`relative flex items-center gap-2.5 px-2 py-1.5 rounded cursor-pointer group ${indent ? "ml-4" : ""} ${active ? "bg-s-sidebar-hover" : "hover:bg-s-sidebar-hover"}`}>
      <MessageSquare size={13} className="sb-icon text-s-sidebar-muted flex-shrink-0" />
      <span className="sb-item flex-1 text-[15px] truncate text-s-sidebar-text">{conv.title}</span>
      <div className="hidden group-hover:flex items-center gap-0.5">
        {projects.length > 0 && (
          <button
