...
export default function AgendaView() {
  ...
  const headerLabel = () => {
    ...
    if (view === "semana") {
      const s = new Date(startOfWeek(cursor)), e = new Date(addDays(startOfWeek(cursor), 6));
  ...
  return (
    <div className="flex flex-col h-full bg-s-bg overflow-hidden">
      ...
      <div className="flex-1 overflow-auto">
        {view === "mes"    && <MonthView  cursor={cursor} events={allEvents} onAdd={d => setModal({ date: d })} onEdit={ev => setModal({ date: ev.date, event: ev })} />}
        {view === "semana" && <WeekView   cursor={cursor} events={allEvents} onAdd={d => setModal({ date: d })} onEdit={ev => setModal({ date: ev.date, event: ev })} />}
      ...
      {modal && (
        <EventModal
          date={modal.date} event={modal.event}
          onClose={() => setModal(null)}
          onSave={async () => { await reload(); setModal(null); }}
          onDelete={async () => { await reload(); setModal(null); }}
        />
      )}
    </div>
  );
...
function MonthView({ cursor, events, onAdd, onEdit }: { cursor: string; events: CalendarEvent[]; onAdd: (d: string) => void; onEdit: (e: CalendarEvent) => void }) {
  const d = new Date(cursor), year = d.getFullYear(), month = d.getMonth();
  const first = firstDayOfMonth(year, month), days = daysInMonth(year, month);
  ...
  for (let dd = 1; dd <= days; dd++)
    cells.push({ iso: `${year}-${String(month+1).padStart(2,"0")}-${String(dd).padStart(2,"0")}`, cur: true });
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="grid grid-cols-7 border-b border-s-border">
        {DAYS_ES.map(d => <div key={d} className="py-2 text-center text-[11px] text-s-muted uppercase tracking-wider">{d}</div>)}
      </div>
      <div className="flex-1 grid grid-cols-7" style={{ gridTemplateRows: `repeat(${cells.length / 7}, 1fr)` }}>
        {cells.map(({ iso, cur }) => {
          ...
          return (
            <div key={iso} onClick={() => cur && onAdd(iso)}
              ...
              <div className="flex items-center justify-between mb-1">
                <span className={`text-[12px] w-6 h-6 flex items-center justify-center rounded-full ${isToday ? "bg-s-accent text-white font-medium" : cur ? "text-s-text" : "text-s-muted"}`}>
              ...
              <div className="space-y-0.5">
                ...
                {dayEvents.length > 3 && <p className="text-[10px] text-s-muted px-1">+{dayEvents.length - 3} más</p>}
              </div>
            </div>
          );
        })}
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="grid border-b border-s-border flex-shrink-0" style={{ gridTemplateColumns: "56px repeat(7, 1fr)" }}>
        <div />
        {days.map(iso => {
          const d = new Date(iso), isToday = iso === todayIso;
          return (
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto">
      <div className="flex items-center justify-between px-6 py-3 border-b border-s-border sticky top-0 bg-s-bg z-10">
        <p className="text-[13px] text-s-muted">{dayEvents.length === 0 ? "Sin eventos" : `${dayEvents.length} evento${dayEvents.length > 1 ? "s" : ""}`}</p>
...
function YearView({ cursor, events, onSelectMonth }: { cursor: string; events: CalendarEvent[]; onSelectMonth: (y: number, m: number) => void }) {
  const year = new Date(cursor).getFullYear(), todayIso = today();
  return (
        ...
        return (
          <div key={month} onClick={() => onSelectMonth(year, month)} className="cursor-pointer group">
            ...
            <div className="grid grid-cols-7 gap-px">
              {DAYS_ES.map(d => <div key={d} className="text-[8px] text-s-muted text-center">{d[0]}</div>)}
              {cells.map((day, i) => {
                if (!day) return <div key={i} />;
                const iso = `${year}-${String(month+1).padStart(2,"0")}-${String(day).padStart(2,"0")}`;
                const hasEvent = events.some(e => e.date === iso), isToday = iso === todayIso;
                return (
                  <div key={i} className="flex items-center justify-center">
                    <span className={`text-[9px] w-4 h-4 flex items-center justify-center rounded-full ${isToday ? "bg-s-accent text-white" : hasEvent ? "bg-s-border text-s-text font-medium" : "text-s-muted"}`}>
                      {day}
                    </span>
                  </div>
                );
...
function EventModal({ date, event, onClose, onSave, onDelete }: {
  ...
  const del = async () => {
    if (!event) return;
    if (confirm("¿Eliminar evento?")) { await eventsDB.delete(event.id); await onDelete(); }
  };
